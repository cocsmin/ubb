...
class ServiceInscrieri:
    ...
    def cauta_persoana_lista_persoane(self, persoanele, pers_curenta, index):
        '''
        ...
        :rtype: bool
        '''
        if index == len(persoanele):
            return False
        else:
            if persoanele[index] == pers_curenta:
                return True
            self.cauta_persoana_lista_persoane(persoanele, pers_curenta, index + 1)
    ...
    def persoane_inscrise_multe_evenimente(self):
        ...
        for index in lista_inscrieri:
            eve = self.get_evenimentele_persoanei(index[0])
            if len(eve) == max and self.cauta_persoana_lista_persoane(pers, index[0], 0) == False:
                pers.append(index[0])
        return pers 

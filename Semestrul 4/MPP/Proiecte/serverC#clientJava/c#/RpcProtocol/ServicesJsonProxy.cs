...
public class ServicesJsonProxy : IProjectServices
{
    private string host;
    private int port;
        ...
        try
        {
            lock (stream)
            {
                ...
                */
                ...
                stream.Write(Encoding.UTF8.GetBytes(requestJson + "\n"));
                stream.Flush();
            }
        }
        catch (Exception e)
    ...
    private void run()
    {
        ...
        while (!finished)
        {
            try
            {
                ...
                Response response = JsonSerializer.Deserialize<Response>(json, jsonOptions);

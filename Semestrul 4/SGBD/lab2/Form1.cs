...
namespace Lab2
{
    public partial class Form1 : Form
    {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                if (dataGridView1.Rows[e.RowIndex].Cells[e.ColumnIndex].Value != null)
                {
                    ...
                    da2.SelectCommand.Parameters.AddWithValue("@ParentId", dataGridView1.Rows[e.RowIndex].Cells[0].Value);
                    ds2.Clear();
                    da2.Fill(ds2);
                    if (ds2.Tables[0].Rows.Count == 0)
                    {
                        MessageBox.Show("Nu există înregistrări corespunzătoare în tabela secundară!");
                    }
        ...
        private void stergereButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Selectați un rând din tabela secundară!");
                return;
            }
            try
            {
        ...
        private void actualizareButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Selectati un meniu", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (panel2.Controls[0].Text == "")
            {
                MessageBox.Show("Selectati o bautura", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            string columnsNames2 = ConfigurationManager.AppSettings["ChildColumsNames"];
            string[] childColumns = columnsNames2.Split(',');
            ...
            {
                childColumns[j] = childColumns[j].Trim();
            }
            ...
            foreach (string param in vars)
            {
                if (param.Trim() == "@id")
                {
                    // Pentru @id, folosim valoarea din dataGridView2 (prima coloană)
                    query = query.Replace(param, dataGridView2.Rows[dataGridView2.CurrentCell.RowIndex].Cells[0].Value.ToString());
                }
                else
                {
                    // Pentru ceilalți parametri, caută poziția în lista de coloane
                    string paramName = param.Trim().TrimStart('@').ToUpper();
                    int index = Array.IndexOf(childColumns, paramName);
                    if (index >= 0)
                    {
                        query = query.Replace(param, panel2.Controls[index].Text);
                    }
                    else
                    {
                        // Afișează o eroare sau log dacă parametrul nu a fost găsit
                        MessageBox.Show("Parametrul " + param + " nu a fost găsit în lista de coloane configurată!");
                    }
                }
            }
            try
        ...
        private void adaugareButton_Click(object sender, EventArgs e)
        {
            ...
            query = query.Replace(vars[vars.Length - 1], panel2.Controls[vars.Length].Text);
